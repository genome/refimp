...
subtest 'fails' => sub{
    ...
    throws_ok(sub{ $pkg->create; }, qr/Mandatory parameters 'contig_data', 'ace', 'clone_name' missing/, 'create w/o params fails');
    throws_ok(sub{ $pkg->create(ace => $ace, clone_name => 'HMPB-AAD13A05'); }, qr/Mandatory parameter 'contig_data' missing/, 'create w/o contig_data fails');
    throws_ok(sub{ $pkg->create(ace => $ace, contig_data => [{}],); }, qr/Mandatory parameter 'clone_name' missing/, 'create w/o contig_data fails');
    throws_ok(sub{ $pkg->create(contig_data => [{}], clone_name => 'HMPB-AAD13A05'); }, qr/Mandatory parameter 'ace' missing/, 'create w/o ace fails');

    throws_ok(sub{ $pkg->create(ace => 'nowaythisexists', contig_data => [{}], clone_name => 'HMPB-AAD13A05'); }, qr/Ace file does not exist\! nowaythisexists/, 'create w/ invalid ace fails');

    throws_ok(sub{ $pkg->create(ace => $ace, contig_data => [], clone_name => 'HMPB-AAD13A05'); }, qr/No contig_data given\!/, 'create w/o actual contig_data fails');
    throws_ok(sub{ $pkg->create(ace => $ace, contig_data => [qw/ 1 1 /], clone_name => 'HMPB-AAD13A05'); }, qr/Not supporting multiple/, 'create w/ >1 contig_data fails');
...
subtest 'create' => sub{
    ...
    my $seq = $sequence->seq;
    ok($seq, 'set seq');
    is($seq->display_id, $clone_name, 'seq display_id');
...
subtest 'create with transposons' => sub{
    ...
    is($transposon_excised_seq->display_id, $clone_name, 'transposon_excised_seq display_id');
