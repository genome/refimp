...
my %prefixes_and_ncbi_names = (
    ...
    TGAC => 'TGMCBa',
    TGAA => 'TG_Ba',
    # Oxytricha 20kb Linear Mitochondrial Clones from Tom Doak
    OXAP => 'OXAP',
    OXAT => 'OXAT',
    OXAV => 'OXAV',
    OXAW => 'OXAW',
...
sub ncbi_name_for_clone_name {
    my ($class, $clone_name) = validate_pos(@_, {isa => __PACKAGE__}, {type => SCALAR});

    unless ( $clone_name =~ m/^([[:upper:]]\w*)-(\w+)([[:upper:]])(\d+)$/x ) {
        return $clone_name
    }
    ...
    my $conversion = $prefixes_and_ncbi_names{$prefix};
    return $clone_name if not $conversion;
